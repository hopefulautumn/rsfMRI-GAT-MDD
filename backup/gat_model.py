...
def fc_matrix_to_graph(fc_matrix: np.ndarray, threshold: float = 0.0) -> Data:
    ...
    Args:
        ...
        threshold: Only keep edges with absolute value > threshold
...
def load_processed_data(
    npz_path: Path = Path('processed/rest_meta_mdd_fc.npz'),
) -> tuple[list[Data], np.ndarray, list[str]]:
    """Load preprocessed data and convert to graph format."""
    ...
    for fc_matrix in fc_matrices:
        graph = fc_matrix_to_graph(fc_matrix, threshold=0.0)
        graphs.append(graph)

    return graphs, labels, group_names
...
def main() -> None:
    ...
    print('Loading data...')
    graphs, labels, group_names = load_processed_data()
    ...
    for fold, result in enumerate(fold_results):
        print(f'Fold {fold + 1}: Acc {result["accuracy"]:.4f} | '
              f'F1 {result["f1"]:.4f} | AUC {result["auc"]:.4f}')

    mean_acc = np.mean([r['accuracy'] for r in fold_results])
    mean_f1 = np.mean([r['f1'] for r in fold_results])
    mean_auc = np.mean([r['auc'] for r in fold_results])
    print(f'Mean: Acc {mean_acc:.4f} ± {np.std([r["accuracy"] for r in fold_results]):.4f} | '
          f'F1 {mean_f1:.4f} ± {np.std([r["f1"] for r in fold_results]):.4f} | '
          f'AUC {mean_auc:.4f} ± {np.std([r["auc"] for r in fold_results]):.4f}')
